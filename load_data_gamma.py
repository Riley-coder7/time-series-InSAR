...
******************************************************************************************************
           ...
           Loading gamma files into h5 file for time series processing.

           usage:
   
                 load_data_gamma.py projectName
      
           e.g.  load_data_gamma.py PacayaT163TsxHhA
...
def main(argv):
    ...
    workDir    = processDir + '/TSSAR'
    if not os.path.isdir(workDir):
        call_str='mkdir ' + workDir
        os.system(call_str)
        
    workDir = workDir + '/TSH5'    
    if not os.path.isdir(workDir):
        call_str='mkdir ' + workDir
        os.system(call_str)
...
################# DEM Define ############
    if 'DEM_dir' in  templateContents:
        DEMDIR =  templateContents['DEM_dir']   
    else:
        SIM = simDir + '/sim_*'
        DEMDIR = glob.glob(SIM)[0]
    
    DPAIR = os.path.basename(DEMDIR).split('_')[1]
    igramDir = glob.glob(processDir + '/*' + projectName + '_' + DPAIR + '*' )[0]
    igramDir = os.path.basename(igramDir)
    
    UTM2RDC = glob.glob(DEMDIR + '/*rlks.UTM_TO_RDC')[0]
    RDCDEM = glob.glob(DEMDIR + '/*rlks.rdc.dem')[0]
    UTMDEM = glob.glob(DEMDIR + '/*rlks.utm.dem')[0]
    UTMPAR = glob.glob(DEMDIR + '/*rlks.utm.dem.par')[0] 
...
####################  start to load data ########################
    ...
    if not os.path.isfile('wrapIfgram.h5'):
        print ''
    ...
    if not os.path.isfile('unwrapIfgram.h5'):
        print ''
    ...
    if not os.path.isfile('coherence.h5'):        
        print ''
...
#########  load DEM and lookup table ##########  
    ...
    if not os.path.isfile('demGeo.h5'):
        print ''
    ...
    if not os.path.isfile('demRdc.h5'):
        print ''
    ...
    if not os.path.isfile('geo2rdc.h5'):
        print ''
